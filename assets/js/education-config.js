...
(function () {
  ...
  function hidePointsCard() {
    // Hide only the visible scoring card, never hide page/body/app containers.
    const cards = qsa('.card');
    cards.forEach(function (card) {
      const text = card.textContent || '';
      const title = card.querySelector('.card-title, .section-title');

      const isPointsCard =
        text.includes('نظام النقاط') ||
        (title && title.textContent && title.textContent.includes('نظام النقاط'));
